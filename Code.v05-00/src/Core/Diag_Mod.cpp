		   ...
		   const float partNum_lost, const float iceMass_lost )
{
    ...
    const NcDim tDim       = currFile.addDim( "t" );
    ...
    //for ( UInt N = 0; N < m.Nx(); N++ ) {
    //    printf("X coordinate %02d: %7.1e\n", N, (m.x())[N]);
    //}
    ...
    if ( outputPDF == 2 ) {
        ...
        #pragma omp critical
        {
        //NcVar var = currFile.addVar( &(partNum_lost), \
        //                 (const char*)charSpc,           \
        //                 tDim, outputType,               \
        //                 (const char*)charUnit,          \
        //                 (const char*)charName );
        ...
        #pragma omp critical
        {
        //NcVar var = currFile.addVar( &(iceMass_lost), \
        //                 (const char*)charSpc,           \
        //                 tDim, outputType,           \
        //                 (const char*)charUnit,          \
        //                 (const char*)charName );
